...
def kustomize_snapshot(snapshot, request, tmp_path):
    def _kustomize_snapshot(example_name):
        ...
        kustomization_path = temp_dir / "kustomization.yaml"
        with open(kustomization_path, "r") as f:
            kustomization_content = f.read()

        # Get the absolute path to the project root and the mealie directory
        project_root = os.path.abspath(".")
        local_manifests_path_abs = os.path.join(project_root, "manifests")

        # Calculate the relative path from the temporary directory to the mealie directory
        ...
        modified_content = re.sub(
            r"https://github.com/adborden/k8s-mealie.git//manifests\?ref=.*",
            relative_path,
